...
function Server(options) {
  ...
  this.cert = rfc3280.Certificate.decode(
      ocsp.utils.toDER(options.cert, 'CERTIFICATE'),
      'der');
  this.cert = this.cert.tbsCertificate;

  var issuerName = rfc3280.Name.encode(this.cert.subject, 'der');
...
Server.prototype.handler = function handler(req, res) {
  ...
  req.on('end', function() {
    ...
    try {
      var ocspReq = rfc2560.OCSPRequest.decode(body, 'der');
    } catch (e) {
...
Server.prototype.getResponse = function getResponse(req, cb) {
  ...
  if (!cert)
    return cb(new Error('Unknown serial: ' + serial));
